  ...
  return (
    <div className="w-278 h-124 relative">
      <button
        ...
        className="w-12 h-12 absolute -left-6 top-54 rounded-full bg-primary-darken flex justify-center items-center cursor-pointer"
      >
        <IoIosArrowBack className="w-6 h-6" />
      </button>
      <button
        onClick={() => setPage(Math.min(page + 1, numberOfPages - 1))}
        className="w-12 h-12 absolute -right-6 top-54 rounded-full bg-primary-darken flex justify-center items-center cursor-pointer"
      >
        <IoIosArrowForward className="w-6 h-6" />
      </button>
      <div className="w-full h-full overflow-hidden">
        <div
          className="w-10000 h-full flex transition-all duration-750"
          style={{ marginLeft: `calc(${-285 * page} * var(--spacing))` }}
        >
          {children}
        </div>
      </div>
